...
autoiniciarScript() {
    ...
    if crontab -l | grep -q "@reboot /etc/vpsmanager/adm.sh"; then
        ...
        if [[ "$opcion" =~ ^[sS]$ ]]; then
            crontab -l | grep -v "@reboot /etc/vpsmanager/adm.sh" | crontab -
        ...
        read -p "Opción: " opcion
        if [[ "$opcion" =~ ^[sS]$ ]]; then
            (crontab -l 2>/dev/null; echo "@reboot /etc/vpsmanager/adm.sh") | crontab -
