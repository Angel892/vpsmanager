...
menuSSH() {
    ...
    # CREAR CUENTA
    opcionMenu -blanco $num "Crear cuenta" false 4
    option[$num]="crear"
    let num++
    ...
    # BLOQUEAR USUARIO
    opcionMenu -blanco $num "Bloquear | Desbloquear"
    option[$num]="bloquear"
    let num++
    ...
    VERY="$(ps aux | grep "${mainPath}/auto/limitador.sh" | grep -v grep)"
    ...
    # LIMITADOR DE CUENTAS
    VERY2="$(ps aux | grep "${mainPath}/auto/desbloqueo.sh" | grep -v grep)"
    [[ -z ${VERY2} ]] && verificar="\e[1;93m[\033[1;31m DESACTIVADO \e[1;93m]" || verificar="\e[1;93m[\033[1;32m ACTIVO \e[1;93m]"
